...
// ConfigName is the file a qubesome config is always called. It is named
// here because both the start path and the command that has to say where
// one was not found spell it.
const ConfigName = "qubesome.config"
...
func Run(opts ...command.Option[Options]) error {
	...
	path := filepath.Join(o.Local, o.Path, ConfigName)
	...
	// So that a later launch with nothing running can offer it. See
	// files.RememberConfig.
...
func StartFromGit(name, gitURL, path, local string, interactive bool) error {
	...
	rel := filepath.Join(path, ConfigName)
